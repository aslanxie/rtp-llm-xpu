...
def _create_local_python_repository(repository_ctx):
  ...
  if _is_windows(repository_ctx):
    ...
    if not python_import_lib_name or not python_import_lib_src:
      # Final fallback: use default CI container image path
      python_import_lib_name = "libpython3.10.so"
      python_import_lib_src = "/opt/conda310/lib/libpython3.10.so"
    python_import_lib_genrule = _symlink_genrule_for_dir(
      repository_ctx, None, '', 'python_import_lib',
      [python_import_lib_src], [python_import_lib_name])
