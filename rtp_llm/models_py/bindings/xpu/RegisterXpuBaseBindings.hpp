    ...
    rtp_ops_m.def("embedding",
                  [](at::Tensor&                     output,
                     ...
                     std::optional<at::Tensor>       text_tokens_mask) {
                      ...
                      TORCH_CHECK(!position_ids.has_value() || position_ids->numel() == 0,
                          "XPU plain embedding ignores position_ids (the fused "
                          "positional path is not implemented). Use embedding_bert "
                          "for positional embeddings.");
                      TORCH_CHECK(!token_type_ids.has_value() || token_type_ids->numel() == 0,
                          "XPU plain embedding ignores token_type_ids. Use "
                          "embedding_bert for BERT token-type semantics.");
    ...
    rtp_ops_m.def("fast_topk_v2",
                  [](at::Tensor& score,
                     ...
                     py::object row_starts) {
                      ...
                      // lengths is int32 (matching CUDA FastTopKParams::lengths).
                      auto len_cpu = lengths.to(at::kCPU).to(at::kInt);
                      auto* len_ptr = len_cpu.data_ptr<int32_t>();
