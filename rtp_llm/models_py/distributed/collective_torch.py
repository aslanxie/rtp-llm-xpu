    ...
    if backend == "xccl":
        ...
        for _gk, _pg in _group_map.items():
            if _pg is not torch.distributed.group.WORLD:
                _gloo_ranks = torch.distributed.get_process_group_ranks(_pg)
                _gloo_pg = torch.distributed.new_group(ranks=_gloo_ranks, backend="gloo", timeout=timedelta(days=36500))
    ...
    if backend == "xccl":
        ...
        for _gk, _pg in _group_map.items():
            if _pg is not torch.distributed.group.WORLD:
                ...
                torch.distributed.broadcast(torch.zeros(1, device=_dev), src=0, group=_pg)
