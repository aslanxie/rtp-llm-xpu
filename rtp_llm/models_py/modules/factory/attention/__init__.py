...
try:
    if device_type == DeviceType.Xpu:
        # Prefer vllm-xpu-kernels flash attention when available
        try:
            from rtp_llm.models_py.modules.factory.attention.xpu_impl.vllm_flash_attn import (
                XpuVllmPrefillImpl,
                XpuVllmDecodeImpl,
            )
            PREFILL_MHA_IMPS.append(XpuVllmPrefillImpl)
            DECODE_MHA_IMPS.append(XpuVllmDecodeImpl)
            logging.info("Registered XPU vllm flash-attention implementations")
        except Exception as _vllm_err:
            logging.info("vllm-xpu-kernels flash attn not available, using SDPA fallback: %s", _vllm_err)

        # Always register SDPA as fallback
