...
class XpuSdpaDecodeImpl(FMHAImplBase):
    ...
    def forward(self, qkv, kv_cache=None, layer_idx=0):
        ...
        if kv_cache is None and num_requests > 1:
            raise RuntimeError(
                f"SDPA decode: kv_cache is None with {num_requests} requests. "
                f"Without KV cache, cross-request attention leakage would occur "
                f"(all tokens attend to each other across requests). "
                f"The no-cache path is only safe for single-request debug/testing.")
